        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h4>Editar {{ ucfirst($item->tipo) }}: {{ $item->nombre }}</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('subsidios_creditos.update', $item->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="tipo" id="tipo" class="form-select" required>
                                ...
                                <option value="subsidio" {{ old('tipo', $item->tipo) == 'subsidio' ? 'selected' : '' }}>Subsidio</option>
                                <option value="credito" {{ old('tipo', $item->tipo) == 'credito' ? 'selected' : '' }}>Crédito</option>
                            </select>
                            @error('tipo')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="orden" id="orden" class="form-control" 
                                   value="{{ old('orden', $item->orden) }}" min="1">
